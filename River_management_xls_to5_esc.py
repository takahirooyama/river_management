...
from pysd.py_backend.statefuls import SampleIfTrue, Delay, Integ
...
def daily_ave_temp_down():
    return _ext_data_daily_ave_temp_down(time())
...
def daily_min_temp_down():
    return _ext_data_daily_min_temp_down(time())
...
def daily_max_temp_down():
    return _ext_data_daily_max_temp_down(time())
...
@component.add(
    ...
    comp_type="Auxiliary",
    comp_subtype="Normal",
    depends_on={"time": 1, "flow_error_sq_sum_d": 1, "flow_variance_sum_d": 1},
)
def nse_d():
    return if_then_else(
        time() < 10,
...
def daily_min_temp_up():
    return _ext_data_daily_min_temp_up(time())
...
def daily_ave_temp_up():
    return _ext_data_daily_ave_temp_up(time())
...
def daily_max_temp_up():
    return _ext_data_daily_max_temp_up(time())
...
def biodiversity():
    """
    2026/04/08 original: 1 - Coefficient for ecosystem * Dam capacity / Forest area storage capacity Dam capacity capacity と ( Forest area storage capacity + Dam capacity )の比によって調整することに変更。
    """
    return 1 - coefficient_for_ecosystem() * dam_capacity() / (
        forest_area_storage_capacity() + dam_capacity()
    )
    ...
    return if_then_else(
        ...
        lambda: if_then_else(
            ...
            lambda: if_then_else(
                ...
                lambda: 0.189,
                lambda: if_then_else(
                    flood_water_level() < 1.5,
                    lambda: 0.253,
                    lambda: if_then_else(
                        flood_water_level() < 2.5,
                        lambda: 0.406,
                        lambda: if_then_else(
                            flood_water_level() < 3.5, lambda: 0.592, lambda: 0.8
                        ),
                    ),
                ),
            ),
...
def inundation_damage_ratio():
    """
    2026/04/09 以下のように設定されていた。 ([(0,0)-(10,1)],(0,0),(0.5,0.2),(1,0.4),(1.8,0.5),(10,1) ) 最新の「治水経済調査マニュアル（案） 令和７年７月 国土交通省 水管理・国土保全局」に沿ってだと、以下のように定義 しなおす。筑後川の下流は勾配が緩いのでAグループ。 ※0m→0.19、0.5m→0.25、1m→0.40、2.0m→0.59、3.0m→0.8 ([(0,0)-(3,0.8)],(0,0.19),(0.5,0.25),(1,0.4),(2,0.59),(3,0.8) ) 2026/04/13 Flood damage ratioと同様に修正。
    """
    return if_then_else(
        ...
        lambda: if_then_else(
            ...
            lambda: if_then_else(
                ...
                lambda: 0.189,
                lambda: if_then_else(
                    innundation_level() < 1.5,
                    lambda: 0.253,
                    lambda: if_then_else(
                        innundation_level() < 2.5,
                        lambda: 0.406,
                        lambda: if_then_else(
                            innundation_level() < 3.5, lambda: 0.592, lambda: 0.8
                        ),
                    ),
                ),
            ),
...
def innundation_risky_area_ratio():
    """
    2026/04/08 小さくすると、levelが増える。一旦変更なし。特に根 がないので1にする（無効化する）ほうが良いかも。1 すると1万件ぐらいになる。標高図を見てザクっと入れ みる？ もともと0.2だったが、流域の下流部は概ね河川氾濫の危 険域（低平地）になっていることから、内水氾濫も多く の地域であり得ると判断して、0.8に変更。
    """
    return 0.8
