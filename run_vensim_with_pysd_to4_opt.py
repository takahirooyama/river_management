...
def aggregate_metrics(res):
    df = res.copy()
    df["year"] = [ _year_index_from_day(day) for day in df.index ]
...
if __name__ == "__main__":
    ...
    # ---- CSV出力 ----
    output_path = Path("data/opt_result_to4.csv")
    rows = []
    rows.append(["score", result.fun])
    rows.append(["calendar_start_year", CALENDAR_START_YEAR])
    rows.append(["calendar_num_years", CALENDAR_NUM_YEARS])
    rows.append(["use_leap_years", USE_LEAP_YEARS])
    rows.append([])
