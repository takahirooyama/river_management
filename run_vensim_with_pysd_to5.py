...
# ---- 入力ファイル設定 ----
#MODEL_MDL = Path("River_management_xls_to5.mdl")  # Vensim テキスト
MODEL_PY = Path("River_management_xls_to5.py")    # 変換済み PySD モデル
...
#    model = read_vensim(MODEL_MDL.as_posix())
    ...
    raise FileNotFoundError(
        #"River_management_xls_to5.py も .mdl も見つかりません。"
        "River_management_xls_to5.py が見つかりません。"
    )

# ---- シミュレーション設定（Vensim の CONTROL に合わせる）----
CALENDAR_START_YEAR = 2009
CALENDAR_NUM_YEARS = 15
USE_LEAP_YEARS = True  # River_management_xls_to5.mdl と合わせる
...
res.to_csv(
    f"data/simulation_output_to5_{SCENARIO}_{RUN_DATE}_rice.csv",
    index_label="day",
)
yearly_summary.to_csv(
    f"data/simulation_output_to5_{SCENARIO}_{RUN_DATE}_rice_yearly_summary.csv",
    index=False,
)
